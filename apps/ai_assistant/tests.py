...
class AIAssistantMockTests(TestCase):
    ...
    def test_mock_answer_contains_management_recommendation(self):
        service = AIAssistantService()
        answer = service.answer("Кто перегружен на этой неделе?", user=self.user)

        self.assertIn("загруз", answer.lower())
        self.assertIn("рекоменд", answer.lower())

    @override_settings(AI_MOCK_MODE=True)
    def test_weekly_report_and_task_extraction_are_structured(self):
        service = AIAssistantService()
        ...
        self.assertIn("Недельный отчёт", report)
        self.assertEqual(extracted["responsible"], "Елена")
        self.assertEqual(extracted["department"], "Финансы")
        self.assertEqual(extracted["priority"], Task.Priority.HIGH)

    @override_settings(AI_MOCK_MODE=False, GEMINI_API_KEY="test-key")
    def test_gemini_provider_failure_falls_back_to_local_answer(self):
        service = AIAssistantService()
    ...
    def test_gemini_prompt_contains_project_task_context(self):
        ...
        self.assertIn("Метрики:", prompt)
        self.assertIn("Задачи:", prompt)
